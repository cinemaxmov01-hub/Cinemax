  ...
  return (
    <div
      ...
      onClick={isPreparing ? undefined : onClick}
        className={`group relative flex-none w-full sm:w-auto cursor-pointer rounded-2xl sm:rounded-3xl overflow-hidden solid-card hover:border-[#39FF14]/40 transition-all duration-300 select-none ${
    ...
    >
      {rank !== undefined && (
        <div
          ...
          className="absolute top-1.5 left-1.5 z-20 flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-lg sm:rounded-xl logo-mark font-sans font-black text-[10px] sm:text-sm"
        >
          {rank}
        </div>
      )}
      ...
      <div id={`poster-container-${movie.id}`} className="relative aspect-[2/3] w-full overflow-hidden bg-neutral-800">
        ...
        <div
          ...
          className="card-hover-overlay absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3 sm:p-4 z-10"
        >
          <button
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="rounded-full logo-mark p-2.5 sm:p-3.5 opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-300">
              <Play className="h-4 w-4 sm:h-5 sm:w-5 fill-black" />
            </div>
          </div>

          <div className="space-y-1 sm:space-y-1.5 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <div className="flex items-center gap-0.5 text-amber-500">
                <Star className="h-2.5 w-2.5 sm:h-3 sm:w-3 fill-amber-500" />
                <span className="text-[10px] sm:text-xs font-bold text-white">{rating}</span>
              </div>
              <span className="text-[9px] sm:text-[10px] font-bold text-neutral-400">•</span>
              <span className="text-[9px] sm:text-[10px] font-medium text-neutral-300">{year}</span>
            </div>
            <h4 className="text-[10px] sm:text-xs font-bold text-white line-clamp-1">{titleText}</h4>
          </div>
        </div>
      </div>

      <div id={`card-footer-${movie.id}`} className="p-2 sm:p-3 space-y-1 surface-elevated border-t border-neutral-800">
        <h4 className="text-[10px] sm:text-xs font-bold text-neutral-200 truncate group-hover:text-[#39FF14] transition-colors duration-200">
          {titleText}
        </h4>
        <div className="flex items-center justify-between text-[9px] sm:text-[10px] text-neutral-500 font-medium">
          <span>{isTv ? "TV Show" : "Movie"}</span>
          <div className="flex items-center gap-1">
            <Star className="h-2 w-2 sm:h-2.5 sm:w-2.5 text-amber-500 fill-amber-500" />
