      ...
      <div id="main-content-panel" className="lg:pl-64 flex flex-col min-h-screen">
        ...
        <header id="top-navbar" className="h-20 glass-navbar sticky top-0 z-40 px-4 lg:px-8 flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0 sm:max-w-lg">
            ...
            {/* Download App Button - animates out when search expands on mobile */}
            <div className={`transition-all duration-300 ease-in-out ${mobileSearchExpanded ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'} lg:hidden`}>
            ...
            <div className={`relative flex-1 min-w-0 sm:w-full sm:flex-none transition-all duration-300 ease-in-out ${mobileSearchExpanded ? 'flex-1' : 'flex-1'}`}>
              {!mobileSearchExpanded && (
                <button
                  onClick={() => setMobileSearchExpanded(true)}
                  className="absolute inset-0 w-full bg-white/5 border border-white/10 rounded-2xl pl-11 pr-12 py-2.5 text-xs text-neutral-400 placeholder:text-neutral-500 focus:outline-none focus:border-[#39FF14]/50 transition-colors text-left cursor-pointer lg:hidden"
                >
              ...
              {mobileSearchExpanded && (
                <>
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-lg hover:bg-white/10 text-neutral-400 hover:text-white transition-colors cursor-pointer lg:hidden"
                  >
