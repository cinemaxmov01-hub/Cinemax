...
// ---------------------------------------------------------------------------
// PREMIUM ACCESS — full admin control over the Premium feature: who is
// Premium and why (their own subscription tier vs. an admin-granted
// override), plus a site-wide kill switch and a list of Premium-exclusive
// titles. This is the authoritative source the website's `user.premium`
// flag is built from (see publicUser() in lib/auth.ts).
// ---------------------------------------------------------------------------

adminRouter.get("/api/admin/premium-access", (_req, res) => {
  const nowMs = Date.now();
  const overrideByUser = new Map(db.data.premium_access.map((p) => [p.user_id, p]));
  const directory = db.data.users
    .map((u) => {
      const override = overrideByUser.get(u.id);
      const overrideActive = Boolean(override) && (!override!.expires_at || new Date(override!.expires_at).getTime() > nowMs);
      ...
      return {
        ...
        overrideGrantedBy: override?.granted_by || null,
        overrideGrantedAt: override?.granted_at || null,
        premium,
        premiumReason: overrideActive ? "admin_override" : u.role === "admin" ? "admin_role" : u.subscription === "Premium" ? "subscription" : "none",
      };
    })
    .sort((a, b) => {
  ...
  res.json({
    total: directory.length,
    premium: directory.filter((u) => u.premium).length,
    free: directory.filter((u) => !u.premium).length,
    users: directory,
    featureEnabled: db.data.site_settings.premiumFeatureEnabled,
    premiumOnlyMovieIds: db.data.site_settings.premiumOnlyMovieIds || [],
  });
});

// Grant (or update) an admin override — independent of the user's own
// subscription tier — with an optional expiry date and note. Passing no
// expiresAt makes the grant permanent until explicitly revoked.
adminRouter.post("/api/admin/premium-access/:userId/grant", (req: AuthedRequest, res) => {
  const userId = req.params.userId;
  const target = getUserById(userId);
  if (!target) return res.status(404).json({ error: "User not found." });
  const { expiresAt, note } = req.body || {};
  const nowIso = new Date().toISOString();
  const existing = db.data.premium_access.find((p) => p.user_id === userId);
  if (existing) {
    existing.granted_at = nowIso;
    existing.granted_by = req.user!.email;
    existing.expires_at = expiresAt || null;
    existing.note = typeof note === "string" ? note.slice(0, 300) : existing.note;
    existing.user_name = target.name;
    existing.user_email = target.email;
  } else {
    db.data.premium_access.push({
      user_id: userId,
      user_name: target.name,
      user_email: target.email,
      granted_at: nowIso,
  ...
  db.save();
  logActivity(req.user!.email, "premium.grant", target.email, { expiresAt: expiresAt || null });
  res.json({ ok: true });
});

adminRouter.delete("/api/admin/premium-access/:userId/revoke", (req: AuthedRequest, res) => {
  const userId = req.params.userId;
  const existing = db.data.premium_access.find((p) => p.user_id === userId);
  if (!existing) return res.status(404).json({ error: "Premium override not found." });
  db.data.premium_access = db.data.premium_access.filter((p) => p.user_id !== userId);
  db.save();
  logActivity(req.user!.email, "premium.revoke", userId);
  res.json({ ok: true });
});

// Site-wide Premium controls: the on/off kill switch and the list of
// movie IDs treated as Premium-exclusive content. Kept as its own
// endpoint (rather than folding into the generic settings PUT) so the
// admin UI can validate/normalize the movie ID list before saving.
adminRouter.put("/api/admin/premium-access/settings", (req: AuthedRequest, res) => {
  const { featureEnabled, premiumOnlyMovieIds } = req.body || {};
  if (featureEnabled !== undefined) {
    db.data.site_settings.premiumFeatureEnabled = Boolean(featureEnabled);
  }
  if (premiumOnlyMovieIds !== undefined) {
    if (!Array.isArray(premiumOnlyMovieIds) || premiumOnlyMovieIds.some((id: any) => !Number.isFinite(Number(id)))) {
      return res.status(400).json({ error: "premiumOnlyMovieIds must be an array of numeric movie IDs." });
    }
    db.data.site_settings.premiumOnlyMovieIds = premiumOnlyMovieIds.map((id: any) => Number(id));
  }
  db.save();
  logActivity(req.user!.email, "premium.settings", "site_settings", { featureEnabled, premiumOnlyMovieIds });
  res.json({
    featureEnabled: db.data.site_settings.premiumFeatureEnabled,
    premiumOnlyMovieIds: db.data.site_settings.premiumOnlyMovieIds,
  });
