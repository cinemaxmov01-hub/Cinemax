...
function normalizeStreamCandidate(raw: string | null, baseUrl: URL): string | null {
  if (!raw) return null;
  let candidate = raw.replace(/\\\//g, "/").replace(/\\u0026/g, "&").trim();
  if (candidate.startsWith("//")) {
    candidate = `${baseUrl.protocol}${candidate}`;
  }
  if (candidate.startsWith("/")) {
    candidate = new URL(candidate, baseUrl.origin).toString();
  }
  try {
    return new URL(candidate, baseUrl.toString()).toString();
  } catch {
    return null;
  }
}

function extractMediaCandidates(text: string, baseUrl: URL): string[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text))) {
      const candidate = normalizeStreamCandidate(match[1] || match[0], baseUrl);
...
async function resolveEmbedMediaUrl(url: string) {
  ...
  const candidates = extractMediaCandidates(text, baseUrl);
  const ordered = Array.from(new Set(candidates)).sort((a, b) => {
    const aMp4 = /\.mp4(?:\?|$)/i.test(a);
    const bMp4 = /\.mp4(?:\?|$)/i.test(b);
    return aMp4 === bMp4 ? 0 : aMp4 ? -1 : 1;
  });
  if (ordered.length === 0) throw new Error("No direct media link found.");

  return { ok: true, sourceUrl: ordered[0], candidates: ordered.slice(0, 5) };
}
...
authRouter.post("/api/stream/full", async (req, res) => {
  ...
  try {
    const result = await resolveEmbedMediaUrl(url);
    return res.json(result);
  } catch (err: any) {
    if (err.message === "Host not permitted for resolving.") {
      return res.status(403).json({ error: err.message });
    }
    console.error("/api/stream/full error:", err);
    return res.status(500).json({ error: err.message || "Failed to resolve full stream." });
  }
});
...
authRouter.post("/api/stream/resolve", async (req, res) => {
  ...
  try {
    ...
    const normalizeCandidate = (raw: string | null): string | null => {
      ...
      let candidate = raw.replace(/\\\//g, "/").replace(/\\u0026/g, "&").trim();
      if (candidate.startsWith("//")) {
        candidate = `${u.protocol}${candidate}`;
      }
      if (candidate.startsWith("/")) {
        candidate = new URL(candidate, u.origin).toString();
      }
      try {
        return new URL(candidate, u.toString()).toString();
      } catch {
        return null;
      }
    };
    ...
    const urlPatterns = [
      ...
      /["'](\/\/[^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,
      /<source[^>]+src=["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,
      /(?:file|src|url|file\S*|source)\s*[:=]\s*["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,
      /["'](https?:\/\/[^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,
      /data\-src=["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,
      /\{\s*file\s*:\s*["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']\s*\}/gi,
    ];
